...
### 2. Molecular cloud
N1d_cloud = 100
spacing_c = (CloudRadius * 2) / N1d_cloud
xc = np.linspace(0.5 - CloudRadius + spacing_c/2, 0.5 + CloudRadius - spacing_c/2, N1d_cloud)
Xc, Yc, Zc = np.meshgrid(xc, xc, xc, indexing='ij')
pos_c_box = np.vstack([Xc.ravel(), Yc.ravel(), Zc.ravel()]).T

# particle inside r = 0.1
...
ids = np.arange(1, N_total + 1) # used in header information
...
omega = math.sqrt(3.0 * beta * G_code * CloudMass / (CloudRadius**3))
# print('omega:', omega)

# calculate relative position (dx, dy)
dx = pos_cloud[:, 0] - center[0]
dy = pos_cloud[:, 1] - center[1]

# set velocity (vx = -omega * y, vy = omega * x)
vel[:N_cloud, 0] = -omega * dy
vel[:N_cloud, 1] = omega * dx



### 6. Write the ic file
filename = "ic_r01_rot001.hdf5"
